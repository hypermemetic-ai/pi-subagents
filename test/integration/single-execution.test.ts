...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("preserves a trusted child's primary failure when its receipt is absent", async () => {
		const profile = { provider: "kimi-coding", model: "k3", effort: "max", serviceClass: "provider-default" } as const;
		mockPi.onCall({ stderr: "child startup failed before receipt\n", exitCode: 23 });
		const agents = [makeAgent("observer", {
			model: "kimi-coding/k3",
			thinking: "max",
			fallbackModels: [],
		...
		assert.equal(result.executionProfile, undefined);
		assert.match(error, /^child startup failed before receipt/m);
		...
		assert.ok(error.indexOf("child startup failed before receipt") < error.indexOf("Secondary trusted execution profile receipt failure"));
	});

	it("preserves a trusted child's logical tool failure before its absent receipt", async () => {
		const profile = { provider: "kimi-coding", model: "k3", effort: "max", serviceClass: "provider-default" } as const;
		mockPi.onCall({ jsonl: [events.toolResult("bash", "command not found\nCommand exited with code 127", true)] });
		const agents = [makeAgent("observer", {
			model: "kimi-coding/k3",
			thinking: "max",
			fallbackModels: [],
		...
		assert.notEqual(result.exitCode, 0);
		assert.match(error, /^bash failed \(exit 127\)/m);
		assert.match(error, /Secondary trusted execution profile receipt failure/);
		assert.ok(error.indexOf("bash failed") < error.indexOf("Secondary trusted execution profile receipt failure"));
	});

	it("preserves a trusted foreground timeout before its absent receipt", async () => {
		const profile = { provider: "kimi-coding", model: "k3", effort: "max", serviceClass: "provider-default" } as const;
		...
		const result = await runSync(tempDir, agents, "observer", "Task", { timeoutMs: 150 });
		const error = result.error ?? "";

		assert.equal(result.timedOut, true);
		assert.match(error, /^Subagent timed out after 150ms\./m);
		assert.match(error, /Secondary trusted execution profile receipt failure/);
		assert.ok(error.indexOf("Subagent timed out") < error.indexOf("Secondary trusted execution profile receipt failure"));
	});
