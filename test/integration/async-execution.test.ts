...
describe("async execution utilities", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("preserves an async child's primary failure when its trusted receipt is absent", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		const profile = { provider: "kimi-coding", model: "k3", effort: "max", serviceClass: "provider-default" } as const;
		mockPi.onCall({ stderr: "async child startup failed before receipt\n", exitCode: 29 });
		const id = `async-profile-child-failure-${Date.now().toString(36)}`;
		executeAsyncSingle(id, {
			agent: "observer",
			task: "Observe",
			agentConfig: makeAgent("observer", {
		...
		assert.equal(payload.success, false);
		assert.equal(payload.results[0]?.exitCode, 29);
		assert.equal(payload.results[0]?.modelAttempts?.[0]?.exitCode, 29);
		assert.equal(status.steps?.[0]?.exitCode, 29);
		assert.equal(payload.results[0]?.executionProfile, undefined);
		assert.equal(status.steps?.[0]?.executionProfile, undefined);
		assert.match(error, /^async child startup failed before receipt/m);
		assert.match(error, /Secondary trusted execution profile receipt failure: .*execution-profile-receipt\.json|Secondary trusted execution profile receipt failure: .*ENOENT/s);
		assert.ok(error.indexOf("async child startup failed before receipt") < error.indexOf("Secondary trusted execution profile receipt failure"));
	});

	it("preserves an async timeout before its absent trusted receipt", { skip: !isAsyncAvailable() ? "jiti not available" : process.platform === "win32" ? "timeout signal delivery intermittent on Windows CI" : undefined }, async () => {
		const profile = { provider: "kimi-coding", model: "k3", effort: "max", serviceClass: "provider-default" } as const;
		mockPi.onCall({ delay: 5_000 });
		const id = `async-profile-timeout-${Date.now().toString(36)}`;
		executeAsyncSingle(id, {
			agent: "observer",
			task: "Observe",
			agentConfig: makeAgent("observer", {
		...
		assert.match(error, /^Subagent timed out after 500ms\./m);
		assert.match(error, /Secondary trusted execution profile receipt failure/);
		assert.equal(status.steps?.[0]?.error, error);
